  ...
  <AdminLayout hide-bottom-nav>
    ...
    <MobilePageHeader title="Detail Transaksi" :subtitle="transaction.transaction_number" back-to="/transactions">
      <template #badge>
        <span
          :class="[
            'text-[11px] px-3 py-1 rounded-full font-semibold border flex items-center gap-1.5',
        ...
        >
          <span
            :class="[
              ...
              isOverpaid || transaction.payment_status === 'lunas' ? 'bg-emerald-400' : 'bg-warning-400 animate-pulse'
            ]"
          ></span>
    ...
    <div class="md:hidden space-y-4 pb-32">
      ...
      <!-- 1. KARTU PELANGGAN & METODE PEMBAYARAN -->
      <section class="bg-white border border-gray-200 rounded-2xl p-4 flex items-center justify-between shadow-sm dark:bg-white/[0.03] dark:border-gray-800">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-brand-500/10 border border-brand-500/20 text-brand-600 dark:text-brand-400 flex items-center justify-center font-bold text-base">
            {{ customerInitial }}
          </div>
          <div>
            <div class="flex items-center gap-2">
              ...
              <span class="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md font-medium dark:bg-white/[0.06] dark:text-gray-300">
                Pelanggan
              </span>
            </div>
        ...
        <div class="text-right shrink-0">
          <span class="text-[11px] text-gray-400 dark:text-gray-500 block">Tanggal</span>
          <span class="text-xs font-semibold text-gray-700 dark:text-gray-200">{{ formatDateShort(transaction.created_at) }}</span>
        </div>
      </section>

      <!-- 2. RINCIAN BARANG -->
      <section class="bg-white border border-gray-200 rounded-2xl p-4 space-y-3 shadow-sm dark:bg-white/[0.03] dark:border-gray-800">
        <div class="flex justify-between items-center text-xs border-b border-gray-100 dark:border-gray-800/60 pb-2.5">
          <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
            Item Pembelian ({{ transaction.items?.length || 0 }})
          </span>
          <span class="text-[11px] font-medium text-gray-400 dark:text-gray-500">Subtotal</span>
        </div>
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex justify-between items-start gap-2">
              <div class="flex-1">
                <p class="text-xs font-semibold text-gray-900 dark:text-slate-100">{{ item.product_name }}</p>
                <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">
                  {{ item.quantity }} unit × <span class="text-gray-600 dark:text-gray-300">{{ formatCurrency(item.price) }}</span>
                </p>
              </div>
              <span class="text-xs font-bold text-gray-900 dark:text-white font-mono">{{ formatCurrency(item.subtotal) }}</span>
            </div>
            <div
      ...
      <!-- 3. TIMELINE RIWAYAT PEMBAYARAN -->
      <section class="bg-white border border-gray-200 rounded-2xl p-4 space-y-3 shadow-sm dark:bg-white/[0.03] dark:border-gray-800">
        <div class="flex justify-between items-center text-xs border-b border-gray-100 dark:border-gray-800/60 pb-2.5">
          <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Riwayat Pembayaran
          </span>
          <span class="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full dark:bg-white/[0.06] dark:text-gray-300">
        ...
        <div class="relative pl-5 space-y-4 text-xs pt-1 before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-[2px] before:bg-gray-200/70 dark:before:bg-gray-700/60">
          <!-- Step 1: Faktur Terbit -->
          <div class="relative">
            <span class="absolute -left-[21px] top-0.5 w-3.5 h-3.5 rounded-full bg-blue-500 ring-4 ring-white dark:ring-[#0b1320]"></span>
            <div class="flex justify-between items-start">
              <div>
                <p class="font-semibold text-gray-800 dark:text-slate-200">Faktur Diterbitkan</p>
                <p class="text-[10px] text-gray-400 dark:text-gray-500">
                  {{ formatDateShort(transaction.created_at) }} &bull; {{ transaction.items?.length || 0 }} Item
                </p>
              </div>
              <span class="font-semibold text-gray-800 dark:text-slate-200 font-mono">{{ formatCurrency(originalSubtotal) }}</span>
            </div>
          </div>
          ...
          <template v-for="activity in sortedActivities" :key="activity.id">
            ...
            <div v-if="activity.type === 'return'" class="relative">
              <span class="absolute -left-[21px] top-0.5 w-3.5 h-3.5 rounded-full bg-rose-500 ring-4 ring-white dark:ring-[#0b1320]"></span>
              <div class="flex justify-between items-start bg-error-500/10 p-2 rounded-xl border border-error-500/20 -ml-2 pl-2">
                <div>
                  <p class="font-bold text-error-600 leading-tight dark:text-error-400">
                ...
                <span class="font-bold text-error-600 dark:text-error-400 font-mono">- {{ formatCurrency(activity.total_refund) }}</span>
              </div>
            </div>
          </template>
        </div>
      ...
      <!-- 4. RINGKASAN TAGIHAN -->
      <section class="bg-white border border-gray-200 rounded-2xl p-4 space-y-2.5 text-xs shadow-sm dark:bg-white/[0.03] dark:border-gray-800">
        <div class="flex justify-between text-gray-500 dark:text-gray-400">
          <span>Total Barang (Kotor)</span>
          <span class="font-mono text-gray-700 dark:text-slate-200">{{ formatCurrency(originalSubtotal) }}</span>
        </div>

        <div v-if="returnAmount > 0" class="flex justify-between text-gray-500 dark:text-gray-400">
          <span>Potongan Retur</span>
          <span class="font-mono text-error-600 dark:text-error-400">- {{ formatCurrency(returnAmount) }}</span>
        </div>

        <div v-if="transaction.discount > 0" class="flex justify-between text-gray-500 dark:text-gray-400">
          <span>Diskon</span>
          <span class="font-mono text-error-600 dark:text-error-400">- {{ formatCurrency(transaction.discount) }}</span>
        </div>

        <div v-if="(transaction.shipping_cost || 0) > 0" class="flex justify-between text-gray-500 dark:text-gray-400">
          <span>Ongkir</span>
          <span class="font-mono text-gray-700 dark:text-slate-200">+ {{ formatCurrency(transaction.shipping_cost) }}</span>
        </div>

        <div class="flex justify-between text-gray-500 dark:text-gray-400">
          <span>Total Bersih Faktur</span>
          <span class="font-mono text-gray-700 dark:text-slate-200">{{ formatCurrency(transaction.total) }}</span>
        </div>

        <div class="flex justify-between text-gray-500 dark:text-gray-400">
          <span>Total Uang Diterima</span>
          <span class="font-mono font-semibold text-emerald-600 dark:text-emerald-400">{{ formatCurrency(totalPaid) }}</span>
        </div>

        <div v-if="transaction.change_amount > 0" class="flex justify-between text-gray-500 dark:text-gray-400">
          <span>Kembalian</span>
          <span class="font-mono text-gray-700 dark:text-slate-200">{{ formatCurrency(transaction.change_amount) }}</span>
        </div>

        <!-- Sisa Tagihan -->
        <div class="border-t border-gray-100 dark:border-gray-800/60 pt-2.5 mt-2 flex justify-between items-center">
          <div>
            <span
              :class="[
                'text-xs font-bold block',
                isOverpaid || effectiveRemaining === 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-warning-600 dark:text-warning-400'
              ]"
            >
              Sisa Tagihan
            </span>
            <span v-if="isOverpaid" class="text-[10px] text-gray-400 dark:text-gray-500">
              Kelebihan bayar {{ formatCurrency(overpayAmount) }}
            </span>
          </div>
          <span
            :class="[
          ...
          >
            {{ isOverpaid || effectiveRemaining === 0 ? 'Rp 0' : formatCurrency(effectiveRemaining) }}
          </span>
        </div>
      </section>
    ...
    <div
      class="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-gray-200 p-3 space-y-2 md:hidden dark:bg-[#0b1320]/95 dark:border-gray-800"
    >
      <!-- Baris 1: Aksi utama — Bayar, WhatsApp, PDF -->
      <div class="grid grid-cols-3 gap-2">
        <button
          v-if="!isOverpaid && transaction.remaining_amount > 0 && transaction.status === 'selesai'"
          @click="showPaymentModal = true"
          class="bg-warning-500 hover:bg-warning-400 text-white font-bold py-2.5 rounded-xl text-xs flex items-center justify-center gap-1.5 transition active:scale-95 shadow-lg shadow-warning-500/10"
        >
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          class="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 rounded-xl text-xs flex items-center justify-center gap-1.5 transition active:scale-95 disabled:opacity-50"
        >
        ...
        <button
          ...
          :disabled="isGeneratingPdf"
          class="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 rounded-xl text-xs flex items-center justify-center gap-1.5 transition active:scale-95 disabled:opacity-50"
        >
      ...
      <div class="grid grid-cols-3 gap-2">
        <button
          ...
          class="bg-gray-100 hover:bg-gray-200 text-gray-600 font-semibold py-2 rounded-xl text-[11px] border border-gray-200 flex items-center justify-center gap-1 active:scale-95 transition dark:bg-white/[0.04] dark:hover:bg-white/[0.08] dark:border-gray-700/60 dark:text-gray-300"
        >
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
          </svg>
    ...
    <ConfirmDialog
      ...
      title="Batalkan Retur?"
      :message="`Apakah Anda yakin ingin membatalkan retur '${returnToDelete?.return_number}'? Stok produk akan dikurangi kembali dan total transaksi dikembalikan seperti semula.`"
...
const handlePrintPdf = async () => {
  ...
  try {
    ...
    const customer = customersStore.customers.find((c) => c.id === transaction.value.customer_id)
    ...
    const invoiceData = {
      ...
      customer_address: customer?.kecamatan || transaction.value.customer_address,
...
const handleShareWhatsApp = async () => {
  ...
  try {
    ...
    const storeSettings = {
      ...
      description: settingsStore.storeSubtitle,
      address: settingsStore.storeAddress,
      email: settingsStore.storeEmail,
      phone: settingsStore.storePhone,
    }
