-- ============================================================
-- Fix Race Condition: Tambah FOR UPDATE lock pada validasi stok
-- Mencegah 2 transaksi simultan mengakibatkan stok negatif
--
-- CRITICAL FIX: Issue #1 - Race Condition Validasi Stok
-- Tanggal: 2026-09-08
-- ============================================================
...
BEGIN
  ...
  -- CRITICAL FIX: Tambah FOR UPDATE untuk lock row produk
  ...
  LOOP
    ...
    FOR UPDATE;  -- Lock baris ini sampai transaction selesai
  ...
  -- ============================================================
  -- PENEGAKAN LIMIT KREDIT (hard stop)
  -- ============================================================
  IF p_customer_id IS NOT NULL AND v_remaining > 0 THEN
    ...
    -- 0 / tidak ada → pakai default global
    ...
    -- 0 lagi → unlimited, tidak ada batasan
    IF COALESCE(v_credit_limit, 0) > 0 THEN
      ...
      IF v_current_debt + v_remaining > v_credit_limit THEN
        RAISE EXCEPTION
          'Limit kredit terlampaui. Limit: Rp %, Hutang saat ini: Rp %, Sisa transaksi ini: Rp %',
  ...
  -- P1 FIX: Pisahkan discount dan return_amount ke kolom terpisah
  INSERT INTO transactions (
    user_id, customer_id, customer_name, subtotal, discount, return_amount, total,
  ...
  -- Loop kedua ini masih aman karena row sudah di-lock di loop pertama
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    SELECT * INTO v_product
    FROM products
  ...
  IF v_paid > 0 THEN
    INSERT INTO transaction_payments (
      ...
      COALESCE(p_payment_method, 'tunai'), NULL, p_transaction_date
    );
  END IF;
