-- Penegakan limit kredit (hard stop) di create_transaction.
-- Blokir transaksi bila sisa (remaining) membuat total hutang customer
-- melewati limit kredit efektif:
--   limit = customers.credit_limit; 0 → fallback store_settings.default_credit_limit;
--   0 lagi → unlimited (tanpa batas).
-- Hutang dihitung dari transactions.remaining_amount > 0 AND status <> 'batal'.
