-- Migration: Update create_transaction dan add_transaction_payment
-- untuk mencatat pembayaran transfer/QRIS ke akun Bank (1-1010)

-- ============================================================
-- UPDATE: create_transaction
-- Logika: pembayaran tunai → Kas, transfer/QRIS → Bank
-- ============================================================
...
BEGIN
  ...
  -- Simpan items + kurangi stok ATOMIK
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    SELECT * INTO v_product
    FROM products
  ...
  -- Auto-jurnal dengan pemilihan akun berdasarkan payment_method
  ...
  IF v_account_kas IS NOT NULL AND v_account_pendapatan IS NOT NULL THEN
    ...
    -- Catat penerimaan ke Kas/Bank sesuai payment_method
...
-- ============================================================
-- UPDATE: add_transaction_payment
-- Logika: pembayaran tunai → Kas, transfer/QRIS → Bank
-- ============================================================
...
BEGIN
  ...
  IF v_receiving_account IS NOT NULL AND v_account_piutang IS NOT NULL THEN
    INSERT INTO journal_entries (user_id, entry_date, description, reference_type, reference_id)
    VALUES (auth.uid(), now(), 'Pembayaran cicilan ' || v_transaction.transaction_number, 'payment', p_transaction_id)
    RETURNING id INTO v_journal_id;

    -- Debit Kas/Bank
    INSERT INTO journal_lines (user_id, journal_id, account_id, account_code, account_name, debit, credit)
    SELECT auth.uid(), v_journal_id, v_receiving_account, v_receiving_code, v_receiving_name, p_amount, 0;
